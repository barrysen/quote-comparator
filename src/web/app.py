...
def _run_job(job_id: str, folder: Path, config_path: Path, job_dir: Path) -> None:
...
@app.get("/api/health")
def health() -> dict:
    key_env = DEFAULT_SETTINGS["llm"]["api_key_env"]
    return {
        "ok": True,
        "llm_configured": bool(os.environ.get(key_env)),
        "llm_key_env": key_env,
...
async def create_job(
    ...
    threading.Thread(
        target=_run_job, args=(job_id, folder, config_path, job_dir), daemon=True
    ).start()
